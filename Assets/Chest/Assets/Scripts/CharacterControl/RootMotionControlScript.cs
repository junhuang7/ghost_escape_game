...
public class RootMotionControlScript : MonoBehaviour
{
    ...
    public GameObject buttonObject; // Declare the buttonObject variable
    ...
    void OnAnimatorMove()
    {
        ...
        Vector3 positionDelta = anim.rootPosition - this.transform.position;
        Quaternion rotationDelta = Quaternion.Inverse(this.transform.rotation) * anim.rootRotation;

        // Scale position and rotation based on our scalars
        Vector3 adjustedPosition = Vector3.LerpUnclamped(Vector3.zero, positionDelta, rootMovementSpeed);
        Quaternion adjustedRotation = Quaternion.LerpUnclamped(Quaternion.identity, rotationDelta, rootTurnSpeed);

        // Apply the adjusted root motion
        rbody.MovePosition(this.transform.position + adjustedPosition);
        rbody.MoveRotation(this.transform.rotation * adjustedRotation);
    }
    ...
    void OnAnimatorIK(int layerIndex)
    {
        if (anim)
        {
            ...
            if (astate.IsName("ButtonPress"))
            {
                ...
                {
                    // Set the character to look at the buttonObject
                    anim.SetLookAtWeight(buttonWeight);
                    anim.SetLookAtPosition(buttonObject.transform.position);
                    
                    // Set the position of the character's right hand to the buttonObject's position
            ...
            {
                // Reset the IK weights when not pressing the button
